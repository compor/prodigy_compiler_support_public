...
bool RangedIndirectionPass::getUserFromInstrTypeRI(Instruction &I, std::vector<Instruction *> &uses, const char *InstrType) {
	bool ret = false;
...
bool RangedIndirectionPass::runOnModule(Module &M) {
	/*
...
void RangedIndirectionPass::getAnalysisUsage(AnalysisUsage &AU) const {
	AU.addRequired<DependenceAnalysisWrapperPass>();
}

char RangedIndirectionPass::ID = 0;
static RegisterPass<RangedIndirectionPass> X("RangedIndirectionPass", "Ranged Indirection for Project",
...
static RegisterStandardPasses Y(
		...
		[](const PassManagerBuilder &Builder,
				legacy::PassManagerBase &PM) { PM.add(new RangedIndirectionPass()); });
